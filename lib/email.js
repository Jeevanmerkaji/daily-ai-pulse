...
import { DEFAULT_TOPIC, TOPICS } from "./topics.js";
...
// Pro subscribers get every topic's story in one email instead of just
// their own chosen one. Each topic gets its own little section, reusing
// the same headline/summary/link shape as the single-story email above.
function buildDigestEmailHtml({ storiesByTopic, unsubscribeUrl }) {
  const sections = TOPICS.map((topic) => {
    const story = storiesByTopic[topic.id];
    if (!story) return ""; // shouldn't happen (the daily job saves all 5), but don't crash if it does
    return `
      <div style="margin-bottom: 28px;">
        <p style="font-size: 12px; color: #71717a; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px;">${topic.label}</p>
        <h2 style="font-size: 19px; line-height: 1.3; margin: 0 0 8px;">${story.headline}</h2>
        <p style="font-size: 15px; line-height: 1.6; margin: 0 0 8px;">${story.summary}</p>
        <a href="${story.source_url}" style="color: #18181b; font-weight: 600; font-size: 14px;">Read the full story at ${story.source_name} &rarr;</a>
      </div>
    `;
  }).join("");
  ...
  return `
    <div style="font-family: Arial, Helvetica, sans-serif; max-width: 560px; margin: 0 auto; color: #18181b;">
      <p style="font-size: 13px; color: #71717a; text-transform: uppercase; letter-spacing: 0.05em;">Daily AI Pulse — Pro Digest</p>
      <h1 style="font-size: 22px; line-height: 1.3; margin: 8px 0 20px;">Today's stories, every topic</h1>
      ${sections}
      <hr style="margin: 8px 0 24px; border: none; border-top: 1px solid #e4e4e7;" />
      <p style="font-size: 12px; color: #a1a1aa;">
        You're getting this because you subscribed to Daily AI Pulse.
        <a href="${unsubscribeUrl}" style="color: #a1a1aa;">Unsubscribe</a>
      </p>
...
// subscribers: rows from the users table (email, unsubscribe_token, topic, plan)
...
export async function sendDailyEmails(storiesByTopic, subscribers) {
  ...
  const emails = subscribers.map((subscriber) => {
    ...
    // Pro subscribers get every topic in one digest email. Everyone else
    // gets the exact same single-story email this app has always sent —
    // this branch changes nothing for free subscribers.
    if (subscriber.plan === "pro") {
      return {
        from: process.env.EMAIL_FROM,
        to: subscriber.email,
        subject: "Your Daily AI Pulse digest — today's top stories",
        html: buildDigestEmailHtml({ storiesByTopic, unsubscribeUrl }),
      };
    }
    ...
    return {
      ...
      html: buildEmailHtml({ story, unsubscribeUrl }),
    };
  });
